...
import { cancelParcel, ParcelServiceError } from "@/services/parcel-service";
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  try {
    const parcel = await cancelParcel(id, parsed.data.reason, user.id, branchId);
    await writeAuditLog({ userId: user.id, action: "PARCEL_CANCEL", entityType: "Parcel", entityId: id, branchId, newData: { reason: parsed.data.reason } });
